...
#[proc_macro]
pub fn async_stream_block(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    impl syn::visit_mut::VisitMut for ReplaceYields {
        fn visit_expr_yield_mut(&mut self, node: &mut syn::ExprYield) {
            ...
            let expr = node.expr.take().unwrap_or(syn::parse_str("()").unwrap());
            node.expr = Some(Box::new(syn::parse2(quote! {
